...
interface MultiSelectProps {
  ...
  zIndex?: number // For controlling z-index in dropdown
  disabled?: boolean
}
...
const MultiSelect: React.FC<MultiSelectProps> = ({
  ...
  React.useEffect(() => {
    ...
    const handleClick = (e: MouseEvent) => {
      // Only close if click is outside this MultiSelect
  ...
  // Sync selectedOptions with defaultSelected prop (controlled mode)
  useEffect(() => {
            ...
            <div className="mb-2 flex h-11 rounded-lg border border-gray-300 py-1.5 pl-3 pr-3 shadow-theme-xs outline-none transition focus:border-brand-300 focus:shadow-focus-ring dark:border-gray-700 dark:bg-gray-900 dark:focus:border-brand-300">
              <div className="flex flex-wrap flex-auto gap-2">
                {selectedValuesText.length > 0 ? (
                  (() => {
                    const visible = selectedValuesText.slice(0, maxVisible)
                    const extra = selectedValuesText.length - maxVisible
                    return (
                      <>
                        {visible.map((text, index) => (
                          <div
                            key={index}
                            className="multi-select group flex items-center justify-center rounded-full border-[0.7px] border-transparent bg-gray-100 py-1 pl-2.5 pr-2 text-sm text-gray-800 hover:border-gray-200 dark:bg-gray-800 dark:text-white/90 dark:hover:border-gray-800"
                          >
                        ...
                        {extra > 0 && (
                          <div
                            className="multi-select group flex items-center justify-center rounded-full border-[0.7px] border-transparent bg-gray-100 py-1 px-3 text-sm text-gray-800 dark:bg-gray-800 dark:text-white/90"
                            key="more"
                          >
                            +{extra}
                          </div>
